...
export class ViewboxComponent implements OnInit {
  ...
  constructor(private activatedRoute: ActivatedRoute) {
    // Retrieves the URL parameter and logs to the console
  ...
  fileInput(event: any) {
    // Show the event type of the listener
  ...
  /** Calculates the percentage at which the lighting should be */
  ...
  /** Canvas image processing for light zones */
  canvasRendering(procesor: string = 'canvasProcessor', result: string = 'canvasResult') {
    // Retrieve the reference to the object that we are going to modify
    ...
    // This is when we load the image, the on load function will get executed after
    img.src = this.imgsrc;
  ...
  /** Canvas image processing for dark zones */
  notCanvasRendering(procesor: string = 'canvasProcessor', result: string = 'canvasResult') {
    // Retrieve the reference to the object that we are gonna modify
    const canvas = <HTMLCanvasElement>document.getElementById(procesor);
    const context = canvas.getContext('2d');
  ...
  renderCanvas(renderX: number = 500, renderY: number = 500) {
    ...
    const canvas = document.getElementById('canvasProcessor') as HTMLCanvasElement;
    const context = canvas.getContext('2d');

    // Here we get reference to the character image loaded in page
    const imageCharacter = document.getElementById('shadow') as any;
